...
export async function toggleChecklistItem(_context: IActionContext, node?: TrialAppChecklistItem): Promise<void> {
    if (node) {
        const state: ITrialAppState | undefined = ext.context.globalState.get(TrialAppState);
        if (state) {
            state.checklist[node.label] = !state.checklist[node.label];
